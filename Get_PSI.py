#This script calculate the PSI inclusion of the reads of the samples using the clusters
# generated by LeafCutter and returns a file with all the PSI values together, removing those clusters with NA values

#The input of this file should be the path where the LeafCutter files are located
...
def main():
    try:
        ...
        bashCommand = "ls "+sys.argv[1]+"/*sorted.gz"
        ...
        for line in cluster_files:
            ...
            output_path = sys.argv[1] + "/" + line.rstrip().split("/").pop()
            file.to_csv(output_path[:-3], sep=" ", index=False,  float_format='%.f', header=False)
            # file.to_csv("/genomics/users/juanluis/SCLC/data/test_LeafCutter/LeafCutter_cluster_results_prueba/prueba.txt.gz",
            #                 sep="\t", index=False,  float_format='%.f', header=False)

        ##################################################
        #2. Remove all the Na rows and merge all the files
        ##################################################

        bashCommand = "ls "+sys.argv[1]+"/*sorted"
        ...
        readCounts = pd.read_table(sys.argv[2], delimiter="\t")
        ...
        sorted_columns.insert(1, "Associated_genes")
        sorted_columns.insert(1, "Type_junction")
        sorted_columns.insert(1, "cluster")
        sorted_columns.insert(1, "end")
        sorted_columns.insert(1, "start")
        sorted_columns.insert(1, "chr")
        ...
        #Save the dataframe
        output_path = sys.argv[1] + "/psi_all_samples.tab"
